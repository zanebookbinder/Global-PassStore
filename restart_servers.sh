...
for HOSTNAME in ${HOSTS} ; do
    ...
    ssh -i ~/.ssh/$USER-keypair $USER@${HOSTNAME} "pkill -u $USER -f '^python3 server.py ${PORTNO}$' && cd project-4---final-project-zane-danny-ahmed && git pull && python3 server.py ${PORTNO} &" &
    echo "server ${IDX} restarted at port ${PORTNO}"
    let IDX=${IDX}+1
done
echo
